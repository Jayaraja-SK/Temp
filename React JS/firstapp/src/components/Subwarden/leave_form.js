...
const ViewAllLeaveFormsSubWarden = () => {
	...
	useEffect(() => {
		axios.get("http://localhost:8080/sub_warden/leave_forms/"+localStorage.getItem("user_id")).then((res) => {
			setData(res.data);
		});
    ...
    const changeStatus = (id) => {
        axios.put("http://localhost:8080/sub_warden/leave_forms/"+localStorage.getItem("user_id")+"/request_id="+id+"&&status="+new_status, data).then((res) => {
...
const ViewLeaveFormsByFilterSubWarden = () => {
    ...
    const filterHandler = (e) => {
		...
		axios.get("http://localhost:8080/sub_warden/leave_forms/"+localStorage.getItem("user_id")+"/status="+filter.status+"&&from_date="+filter.from_date+"&&to_date="+filter.to_date).then((res) => {
			setData(res.data);
		});
	}
