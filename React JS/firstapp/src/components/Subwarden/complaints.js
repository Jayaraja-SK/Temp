...
const ViewAllComplaintsSubWarden = () => {
    ...
    const filterHandler = (e) => {
		...
		axios.get("http://localhost:8080/sub_warden/complaints/"+localStorage.getItem("user_id")+"/complaint_type="+filter.complaint_type+"&&status="+filter.status+"&&from_date="+filter.from_date+"&&to_date="+filter.to_date).then((res) => {
            setData(res.data);
		});
	}
